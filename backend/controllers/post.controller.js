...
export const createPost = async (req, res) => {
  try{
    ...
    if(!text && !img){
      return res.status(400).json({msg: "Post can't be empty"});
    }
...
export const getAllPosts = async (req, res) => {
  try{
    const posts = await Post.find().sort({createdAt: -1}).populate({path: "user", select: "-password"}).populate({path: "comments.user", select: "-password"}).populate({path: "likes", select: "-password"});
    if(posts.length === 0){
